...
SDRunoPlugin_RemoteBridgeForm::SDRunoPlugin_RemoteBridgeForm(SDRunoPlugin_RemoteBridgeUi &parent, IUnoPluginController &controller) : nana::form(nana::API::make_center(formWidth, formHeight), nana::appearance(false, true, false, false, true, false, false)), m_parent(parent), m_controller(controller) {
	Setup();
}


// Form deconstructor
SDRunoPlugin_RemoteBridgeForm::~SDRunoPlugin_RemoteBridgeForm() {

}


// Start Form and start Nana UI processing
void SDRunoPlugin_RemoteBridgeForm::Run() {
	show();
	nana::exec();
}
...
void SDRunoPlugin_RemoteBridgeForm::Setup() {
	...
	hModule = GetModuleHandle("SDRunoPlugin_RemoteBridge");
	hdc = GetDC(NULL);
	...
	*(BITMAPINFO *)(settoverPixels + sizeof(BITMAPFILEHEADER)) = bmInfo_sett_over;
	...
	ReleaseDC(NULL, hdc);
	bg_border.load(img_border, nana::rectangle(0, 0, 400, 300));
	bg_border.stretchable(0, 0, 0, 0);
	bg_border.transparent(true);
	bg_inner.load(img_inner, nana::rectangle(0, 0, 392, 259));
	...
	title_bar_label.caption("< bold size = 6 color = 0x000000 font = \"Verdana\">BRIDGE</>");
	...
	min_button.events().mouse_down([&] {
		min_button.load(img_min_down, nana::rectangle(0, 0, 20, 15));
	});
	min_button.events().mouse_up([&] {
		min_button.load(img_min_normal, nana::rectangle(0, 0, 20, 15));
		nana::API::zoom_window(this->handle(), false);
	});
	min_button.events().mouse_leave([&] {
		min_button.load(img_min_normal, nana::rectangle(0, 0, 20, 15));
	});
	...
	close_button.events().mouse_down([&] {
		close_button.load(img_close_down, nana::rectangle(0, 0, 20, 15));
	});
	close_button.events().mouse_up([&] {
		close_button.load(img_close_normal, nana::rectangle(0, 0, 20, 15));
		close();
	});
	close_button.events().mouse_leave([&] {
		close_button.load(img_close_normal, nana::rectangle(0, 0, 20, 15));
	});
	...
	//port label
	portLbl.caption("Serial Port:");
	portLbl.fgcolor(nana::color_rgb(forecolor));
	portLbl.transparent(true);

	m_parent.EnumeratePorts();
	StringVector portsList = m_parent.ports;

	//port combobox
	for (UINT i = 0; i < portsList.size(); i++) {
#if 0
		std::wstring port_w_string(portsList[i]);                            //convert to wstring
#else
		std::string port_w_string(portsList[i]);
#endif
		std::string port_string(port_w_string.begin(), port_w_string.end()); //and convert to string.
		portCb.push_back(port_string);
	}
	//portCb.option(1);
	...
	startBtn.events().click([&] {
		statusLbl.caption(portCb.text(portCb.option()));

		if (!m_parent.IsRunning()) {
#ifdef DEBUG
			OutputDebugStringA("Attempt to run bridge");
#endif
			statusLbl.caption("Running");
		} else {
#ifdef DEBUG
			OutputDebugStringA("Attempt to stop bridge");
#endif
			statusLbl.caption("Stopped");
		}

		m_parent.StartButtonClicked(portCb.text(portCb.option()));
	});
	//portCb.ext_event().selected = [] { std::cout << "selected a new text" << std::endl; };
...
//void SDRunoPlugin_RemoteBridgeForm::SettingsButton_Click()
//{
